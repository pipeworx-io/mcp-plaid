 ...
 *
 * Tools:
 * - get_accounts: list linked bank accounts
 * - get_transactions: fetch transactions for a date range
 * - get_balance: get real-time account balances
 *
 * Requires Plaid client_id and secret passed as _apiKey in "client_id:secret" format.
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Plaid');
}
...
const tools: McpToolExport['tools'] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        access_token: {
          type: 'string',
          description: 'Plaid access token for the linked institution (access-sandbox-xxx)',
        },
        _apiKey: {
          type: 'string',
          description: 'Plaid credentials as "client_id:secret" (e.g. "abc123:def456")',
        },
      },
      required: ['access_token', '_apiKey'],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        access_token: {
          type: 'string',
          description: 'Plaid access token for the linked institution',
        },
        start_date: {
          type: 'string',
        ...
        _apiKey: {
          type: 'string',
          description: 'Plaid credentials as "client_id:secret"',
        },
      },
      required: ['access_token', 'start_date', 'end_date', '_apiKey'],
    },
  },
  {
    name: 'get_balance',
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    case 'get_accounts':
      return getAccounts(clientId, secret, args.access_token as string);
    ...
    case 'get_balance':
      return getBalance(clientId, secret, args.access_token as string);
    default:
      throw new Error(`Unknown tool: ${name}`);
  }
}

async function plaidPost(endpoint: string, clientId: string, secret: string, body: Record<string, unknown>) {
  const res = await pwFetch(`${BASE_URL}${endpoint}`, {
    method: 'POST',
  ...
  if (!res.ok) throw await httpError(res, 'Plaid API error');
  return (await res.json()) as Record<string, unknown>;
}

async function getAccounts(clientId: string, secret: string, accessToken: string) {
  return plaidPost('/accounts/get', clientId, secret, { access_token: accessToken });
}
...
async function getTransactions(
  ...
  return plaidPost('/transactions/get', clientId, secret, {
...
async function getBalance(clientId: string, secret: string, accessToken: string) {
  return plaidPost('/accounts/balance/get', clientId, secret, { access_token: accessToken });
}

export default { tools, callTool, meter: { credits: 5 }, provider: 'plaid' } satisfies McpToolExport;
